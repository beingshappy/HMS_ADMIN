...
import { FaTrashAlt, FaEdit, FaSave, FaTimes, FaFileUpload } from 'react-icons/fa';

const Radiology = ({ sidebarCollapsed }) => {
  const [data, setData] = useState([
    { id: 1, title: 'Chest X-Ray', type: 'X-Ray', date: '2025-08-02', uploadedBy: 'Dr. Smith' },
    { id: 2, title: 'MRI Brain', type: 'MRI', date: '2025-08-05', uploadedBy: 'Dr. John' },
    { id: 3, title: 'CT Abdomen', type: 'CT Scan', date: '2025-08-08', uploadedBy: 'Dr. Alice' },
  ]);
  ...
  return (
    ...
    >
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
          <h2 style={{ color: '#2c3e50', fontWeight: '600' }}>Radiology</h2>
          <p className="text-muted mb-0">Manage X-rays, scans, and reports.</p>
        </div>
        <button
      ...
      <input
        ...
        style={{ padding: '8px', borderRadius: '8px', maxWidth: '300px', marginBottom: '20px' }}
      />
      ...
      <div style={{ background: '#fff', borderRadius: '12px', padding: '20px', overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead style={{ background: '#f1f5f9', cursor: 'pointer' }}>
            <tr>
              <th style={{ padding: '12px' }} onClick={() => requestSort('title')}>
                TITLE {sortConfig.key === 'title' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
              </th>
              <th style={{ padding: '12px' }} onClick={() => requestSort('type')}>
                TYPE {sortConfig.key === 'type' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
              </th>
              <th style={{ padding: '12px' }} onClick={() => requestSort('date')}>
                DATE {sortConfig.key === 'date' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
              </th>
              <th style={{ padding: '12px' }} onClick={() => requestSort('uploadedBy')}>
          ...
          <tbody>
            {currentData.map(item => (
              <tr key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={{ padding: '12px' }}>
                  {editingId === item.id ? (
                    <input value={editValues.type} onChange={e => setEditValues({ ...editValues, type: e.target.value })} />
                  ) : (
                    item.type
                  )}
                </td>
                <td style={{ padding: '12px' }}>
                  {editingId === item.id ? (
                    <input value={editValues.date} onChange={e => setEditValues({ ...editValues, date: e.target.value })} />
                  ) : (
                    item.date
                  )}
                </td>
                <td style={{ padding: '12px' }}>
                  {editingId === item.id ? (
                    <input value={editValues.uploadedBy} onChange={e => setEditValues({ ...editValues, uploadedBy: e.target.value })} />
                  ) : (
                    item.uploadedBy
                  )}
                </td>
      ...
      {adding && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ background: '#fff', padding: '30px', borderRadius: '12px', minWidth: '300px' }}>
            <h3 style={{ marginBottom: '20px' }}>Add New Radiology Report</h3>
